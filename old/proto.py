...
with PdfPages('C:/Users/limhs/Desktop/job_report_' + jobname + '.pdf') as pdf:
    ...
    cmax = int(np.max(H))
    cmesh = ax1.imshow(H.T, cmap=plt.get_cmap('viridis', cmax + 1), interpolation='nearest', origin='lower',
                       extent=[binEdge_east[0], binEdge_east[-1], binEdge_north[0], binEdge_north[-1]])
